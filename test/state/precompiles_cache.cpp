...
{
std::optional<qrvmc::Result> Cache::find(PrecompileId id, bytes_view input, int64_t gas_left) const
{
    if (const auto& cache = m_cache.at(stdx::to_underlying(id)); !cache.empty())
    {
        ...
        if (const auto it = cache.find(input_hash); it != cache.end())
        {
            if (const auto& o = it->second; !o.has_value())
                return qrvmc::Result{QRVMC_PRECOMPILE_FAILURE};
            else
                return qrvmc::Result{QRVMC_SUCCESS, gas_left, 0, o->data(), o->size()};
        }
    }
    return {};
}

void Cache::insert(PrecompileId id, bytes_view input, const qrvmc::Result& result)
{
...
{
    const auto stub_file = std::getenv("QRVMONE_PRECOMPILES_STUB");
    ...
    try
    {
        ...
        for (size_t id = 0; id < j.size(); ++id)
        {
            ...
            {
                auto& e = cache[qrvmc::from_hex<hash256>(h_str).value()];
                if (!j_input.is_null())
                    e = qrvmc::from_hex(j_input.get<std::string>());
            }
        }
    }
    catch (...)
    {
        std::cerr << "qrvmone: Loading precompiles stub from '" << stub_file << "' has failed!\n";
    }
}

Cache::~Cache() noexcept
{
    const auto dump_file = std::getenv("QRVMONE_PRECOMPILES_DUMP");
    ...
    {
        std::cerr << "qrvmone: Dumping precompiles to '" << dump_file << "' has failed!\n";
    }
}
}  // namespace qrvmone::state
